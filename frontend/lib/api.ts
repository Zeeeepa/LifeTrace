...
export const api = {
	...
	getScreenshotImage: (id: number) =>
		`${API_BASE_URL}/api/screenshots/${id}/image`,
	...
	eventSearch: (params: { query?: string; limit?: number }) =>
		apiClient.post("/api/event-search", params),

	eventSemanticSearch: (params: { query: string; top_k?: number }) =>
	...
	// 流式聊天（带事件上下文）
	sendChatMessageWithContextStream: async (
		params: {
			message: string;
			conversation_id?: string;
			event_context?: Array<{ event_id: number; text: string }>;
		...
		const response = await fetch(
			`${API_BASE_URL}/api/chat/stream-with-context`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	// 创建新会话
	createNewChat: (_chatType?: string, _contextId?: number) =>
	...
	// 应用使用分析
	getAppUsage: (params?: { start_date?: string; end_date?: string }) =>
		apiClient.get("/api/app-usage", { params }),

	// 时间分配
	getTimeAllocation: (params?: { start_date?: string; end_date?: string }) =>
	...
	saveConfig: (config: Record<string, unknown>) =>
		apiClient.post("/api/save-config", config),
	...
	batchDeleteTasks: (projectId: number, taskIds: number[]) =>
		apiClient.post(`/api/projects/${projectId}/tasks/batch-delete`, {
			task_ids: taskIds,
		}),
	...
	getTaskProgress: (
		...
		apiClient.get(`/api/projects/${projectId}/tasks/${taskId}/progress`, {
			params,
		}),
	...
	generateTaskSummary: (projectId: number, taskId: number) =>
		apiClient.post(
			`/api/projects/${projectId}/tasks/${taskId}/generate-summary`,
		),
	...
	pauseAllSchedulerJobs: () => apiClient.post("/api/scheduler/jobs/pause-all"),
	...
	getCostConfig: () => apiClient.get("/api/cost-tracking/config"),
	...
	createActivityFromEvents: (eventIds: number[]) =>
		apiClient.post("/api/activities/manual", { event_ids: eventIds }),
};
